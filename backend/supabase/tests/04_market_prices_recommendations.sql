...
SELECT tests.expect_eq('authenticated: cannot UPDATE crop_recommendations',
  $$UPDATE public.crop_recommendations SET status = 'completed' RETURNING id$$, '');
SELECT tests.expect_eq('authenticated: cannot DELETE crop_recommendations',
  $$DELETE FROM public.crop_recommendations RETURNING id$$, '');
SELECT tests.reset_identity();
